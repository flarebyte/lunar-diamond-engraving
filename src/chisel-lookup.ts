import {type EngravingChisel} from './api-model.js';

export const getLogger = (chisel: EngravingChisel, name: string) => {
  const defaultFunc = chisel.loggerFunctions[name];
  if (defaultFunc !== undefined) {
    return defaultFunc;
  }

  throw new Error(`${name} is not available as a logger (825309)`);
};
...
export const getUses = (chisel: EngravingChisel, name: string) => {
  ...
  throw new Error(`${name} is not available as an action uses (589684)`);
};
...
export const geOnFinishtUses = (chisel: EngravingChisel, name: string) => {
  ...
  throw new Error(`${name} is not available as an OnFinish uses (848649)`);
};
